...
sub move {
  ...
  local *zone = $parea == 2 ? *boti : $parea == 3 ? *hand : ($parea == 4 || $parea == 5) ? *deck : "";
  local *arr = $varea == 1 ? *boti : $varea == 0 ? *hand : *deck;
...
sub del_move {  # 手札、山札、墓地の不要なスペースを削除
  &del_null(sprintf("%s", $varea == 0 ? *hand : $varea == 1 ? *boti : *deck), $vside);
}
...
#          : $c_name[$cno] eq "竜魔神王バルカディア・NEX" ? (11,41,50)
    ...
    if ($c_name[$cno] =~ /天神|海神|黒神|炎神|地神/) {
      ...
      @god = $c_name[$cno] eq "G・A・ペガサス" ? ("1713")
         ...
         :$c_name[$cno] eq "神王タイタス" ? ("2621")
         :$c_name[$cno] eq "イズモ" ? ("3968","3969","3978","3979","4007","4008","4009","4010","4026","4027","4048","4049")
