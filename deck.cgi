...
# if($cou == $maxdeck){
    ...
    $dnam[$usedeck] = $dnam;
    $dcon[$usedeck] = $dcon;
    $P{"deck$usedeck"}="$dnam[$usedeck]\-$dcon[$usedeck]";
# }
  $P{'c_dname'} = $P{'url'} = "";
  &pfl_write($id);
}
  ...
  print <<"EOM";
    ...
    <select name="series">
        ...
        <optgroup label = "特別なカード">
            ...
            <option value="82" $selstr[82]>プロモーションカード</option>
...
sub cardsort {
  ...
  $selstr6[$F{'sortkey1'}] = $selstr7[$F{'sortkey2'}] = $selstr8[$F{'sortkey3'}] = $selstr9[$F{'sortkey4'}] = " selected";
}
...
sub deckread{
  ...
  my $cou = 0;
  foreach my $i(1 .. $maxdeck){
    if(!($P{"deck$i"})){ $dnam[$i] = "記録なし"; @{$deck[$i]} = (); next; }
    else{
      ($dnam[$i],$dcon[$i]) = split(/-/,$P{"deck$i"});
      $cou = $i;
    }
  }
