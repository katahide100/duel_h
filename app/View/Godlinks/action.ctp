...
# &com_error("$phasestr[$phase]にはマナゾーン以外にカードを移動することはできません") if $phase == 2 && $parea != 0;
  ...
  if ( $parea == 9 ) {
    ...
    if ( $F{'decktop'} ) {
      ...
      com_error("サイキック・クリーチャー以外のカードを超次元ゾーンに移動することはできません")  unless (syu_chk($cardno, 145) || syu_chk($cardno, 150) || syu_chk($cardno, 103) || syu_chk($cardno, 119));
...
sub put_card_sub {
  ...
  $parea == 0 ? "マナゾーン" : "シールド",
  $parea == 0 ? "出した。" : "セット！");
  if (syu_chk($cardno, 145) || syu_chk($cardno, 150) || syu_chk($cardno, 103) || syu_chk($cardno, 119))  {
...
sub move_sub {
  my $side = $_[0];
  &p_mess;
  if ((syu_chk($cardno, 145) || syu_chk($cardno, 150) || syu_chk($cardno, 103) || syu_chk($cardno, 119)) && ($parea != 9)) {
    s_mes("《$c_name[$cardno]》は超次元ゾーンに送られた。");
  ...
  if ($F{'run_select'}) {
    ...
    if ($S{'p'} eq "shield") {
      ...
      for (my ($i) = 0; $i < scalar @card; $i++) {
        if (&syu_chk($card[$i], 145) || &syu_chk($card[$i], 150) || &syu_chk($card[$i], 103) || &syu_chk($card[$i], 119)) {
